...
uint32_t *pulLowerCSA = NULL;
	...
	_mtcr( SYSCON_ADDR, portINITIAL_SYSCON );
	_isync();
	...
	ulMFCR = _mfcr( PSW_ADDR );
	ulMFCR &= portRESTORE_PSW_MASK;
	_dsync();
	_mtcr( PSW_ADDR, ulMFCR );
	_isync();
...
uint32_t uxReturn = 0UL;
	...
	_mtcr( ICR_ADDR, ( ( uxReturn & ~portCCPN_MASK ) | configMAX_SYSCALL_INTERRUPT_PRIORITY ) );
	_isync();
	_enable();
